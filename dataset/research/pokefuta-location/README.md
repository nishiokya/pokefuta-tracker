...
- 調査結果の`candidate.tags`は、今回の根拠で確認したタグであり、既存タグ全体の置換指示ではない。
- 正本への統合は既存タグとのmerge（追加）を既定とする。
- 既存タグを削除するのは、監査結果に対象タグと削除理由が明記されている場合に限る。
- 調査範囲外の`geography`、`relation`、`data_quality`タグを巻き添えで削除しない。
- タグを置換する必要がある場合は、追加タグと削除タグを監査記録で明確に分ける。
