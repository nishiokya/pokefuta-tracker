...
`near_gundam_manhole` は近接判定用のタグに留め、旅行中の体験情報は
`dataset/gundam_manhole_spots.json` で管理する。

- `rank`: 「一緒に巡れるスポット」の表示順位。距離順ではなく体験価値を優先する
- `headline` / `story`: サマリー特集とSNS投稿候補で共用する蘊蓄
- `proximity_label`: 「同じ道の駅」「入口の反対側」「徒歩10秒」など現地感のある案内
- `onsite_tip`: 見落とし注意、入口の反対側、同施設内など現地で役立つ情報
- `experience_tags`: 体験の特徴を短いラベルで表示する
- `distance_m`: 検証用の参考値。主表示やランキング基準にはしない

新しい組み合わせを目視確認したら、ポケふたIDとガンダムマンホールIDを紐づけ、
現地での移動や見つけ方が伝わる文面を追加する。
---
...
ユーザーが編集する唯一のファイル。称号・マンホール詳細・自治体公式URLを1ファイルに集約している。
構造は4ブロック:
...
`building` は図鑑の詳細ページで設置場所としてそのまま表示されるので、表記を揃える:

| ルール | 例 |
|---|---|
| スペースは半角1つ（全角・連続スペースは使わない） | `指宿警察署 指宿中央交番` |
| 施設名と場所の詳細は半角スペースで区切る。「前」1文字だけなら続けて書く | `浜益温泉 玄関前` / `関シティターミナル前` |
| 「内」「敷地内」は付けない | `豊橋公園`（`豊橋公園内` としない） |
| 自治体名は先頭に付けない（図鑑に別に表示される）。施設の正式名に含まれる場合は残す | `指宿図書館` / `鈴鹿市伝統産業会館` |
| 道の駅は国交省の登録名、駅前は「〇〇駅前」 | `道の駅上ノ国もんじゅ` / `高知駅前` |
| 愛称は「」、補足は全角の（） | `下呂市観光交流センター「湯めぐり館」` / `海岸公園（藤塚地区）` |
...
`SCHEMA.md` の「`dataset/manhole_titles.json` 由来のフィールド」節に本ファイルへの参照と、`islands` / `vocabulary` の編集が PR 化される運用を明記済み。
