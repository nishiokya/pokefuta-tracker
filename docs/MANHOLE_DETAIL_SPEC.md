...
| 6 | 近くのデザインマンホール | 20km以内のガンダム蓋・キャラクターマンホール・写真館投稿（最大5件・距離付き）。写真館に投稿がある蓋は `pokefuta.com/design-manholes/{id}` へ直リンク＋写真、無い蓋は一覧へフォールバック＋「📷 写真募集中」 | 共通（tracker実装済み） | pokefuta.com は今後 |
| 7 | 次に寄れるポケふた | 距離付き近隣リスト（5件） | 共通 | 共通 |
| 8 | 登場ポケモン | 4言語名・タイプ・世代・「同じポケふたを見る」（検索向け情報のため旅情報の下に配置） | 共通 | 共通 |
| 9 | 同じポケモンのポケふた | 全国の同ポケモン設置 | 共通 | 共通 |
| 10 | 県内のポケふた | 同県リスト | 共通 | 共通 |
| 11 | リンク・共有 | X/LINE/Web Share、公式サイト、県マップ、全国マップ | 共通 | 共通 |
| 12 | コメント | 閲覧＋投稿 | pokefuta.com のみ（閲覧可、投稿はログイン） | pokefuta.com のみ |

- セクション6のデータソース: `docs/gmanhole.ndjson` / `dataset/aichi_character_manholes.ndjson`（active＋installed＋公式系座標のみ）/ `docs/design_manholes.ndjson`（写真館投稿。canonical_ref / nearby_refs でマスタ蓋に合流し、写真館詳細リンクと写真として使う。ポケふた至近の投稿は除外）。🎨近接称号バッジは本文の `#design-manholes` アンカーにリンクする。

- tracker は 1–11 を静的HTMLに焼き込む（現行どおり日次）。
- pokefuta.com は同じ順序・同じ見出しで描画し、12 とログインレイヤーを追加する。
